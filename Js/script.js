    ...
    function showPost(post) {
      ...
      elem.innerHTML = "<h2 class=\"title\">" + post.title + "</h2>" + "<p>Published on " + post.publishedOn + " by " + post.user + "</p> <hr>" + post.content;
      
      if(loggedin == true)
        if(post.user == firebase.auth().currentUser.email)
        {
          elem.innerHTML += '<button id="'+ post.id +'" >Delete</button>';
        }
      ...
      if(loggedin == true)
        if(post.user == firebase.auth().currentUser.email)
        {
          document.getElementById(post.id).addEventListener('click', function() {deletePost(post.id)});
        }
    ...
    function publishPost(e) {
        ...
        if(document.getElementById('title').value != "")
        {
            ...
            let postsRef = firebase.database().ref('posts/').push();
            postsRef.set({
              id: postsRef.key,
              title: title,
              content: content,
